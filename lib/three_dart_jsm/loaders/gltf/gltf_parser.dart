...
class GLTFParser {
  ...
  parse(onLoad, onError) async {
    ...
    _markDefs();
    // _invokeAll((ext) {
    //   if (ext.runtimeType == GLTFParser ||
    //       ext.runtimeType == GLTFLightsExtension) {
    ...
    if (byteStride != null && byteStride != itemBytes) {
      ...
      if (ib == null) {
        ...
        ib = InterleavedBuffer(
            Float32Array.fromList(array), (byteStride / elementBytes).round());

        parser.cache.add(ibCacheKey, ib);
      }

      bufferAttribute = InterleavedBufferAttribute(ib, itemSize,
          ((byteOffset % byteStride) / elementBytes).round(), normalized);
    } else {
      if (bufferView == null) {
